...
mod identity_tests {
    #[test]
    fn generated_permissions_keep_the_official_plugin_namespace() {
        let reference = include_str!("../permissions/autogenerated/reference.md");
        let bridge = include_str!("bridge.js");

        assert!(reference.contains("`mcp-bridge:allow-execute-command`"));
        assert!(!reference.contains("hushlor-mcp-bridge"));
        assert!(bridge.contains("plugin:mcp-bridge|"));
        assert!(!bridge.contains("plugin:hushlor-mcp-bridge|"));
    }
}
