...
function verifyRuntimeIdentity() {
   ...
   const buildScript = readFileSync(join(rootDir, packages.plugin.path, 'build.rs'), 'utf8');

   const library = readFileSync(join(rootDir, packages.plugin.path, 'src/lib.rs'), 'utf8');

   const permissions = readFileSync(join(rootDir, packages.plugin.path, 'permissions/autogenerated/reference.md'), 'utf8');

   const hasOfficialLinks = /^links = "tauri-plugin-mcp-bridge"$/m.test(cargo);

   const hasBuildOverride = buildScript.includes('const RUNTIME_PLUGIN_PACKAGE_NAME: &str = "tauri-plugin-mcp-bridge"') &&
      buildScript.includes('set_var("CARGO_PKG_NAME", RUNTIME_PLUGIN_PACKAGE_NAME)');

   const hasOfficialBuilder = library.includes('PluginBuilder::<R>::new("mcp-bridge")');

   const hasOfficialPermissions = permissions.includes('`mcp-bridge:allow-execute-command`');

   const hasNoHushlorNamespace = !permissions.includes('hushlor-mcp-bridge');

   if (!(hasOfficialLinks && hasBuildOverride && hasOfficialBuilder && hasOfficialPermissions && hasNoHushlorNamespace)) {
      throw new Error('Runtime plugin identity must remain the official mcp-bridge namespace');
   }
}
...
function verifyRelease(version) {
   ...
   if (cargo.name !== 'tauri-plugin-hushlor-mcp-bridge' || cargo.version !== version || cargo.links !== 'tauri-plugin-mcp-bridge') {
